    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '🎭 ROLE COMMANDS (4 commands)', value: '```\n' +
                '!giverole <userID> <roleID> - Give a role to user\n' +
                '!removerole <userID> <roleID> - Remove role from user\n' +
                '!roles [userID] - Show user\'s roles\n' +
        ...
        .setFooter({ text: `⚠️ Moderation & Role commands require <@&${MOD_ROLE_ID}> role` })
        .setTimestamp();
...
client.on('messageCreate', async (message) => {
    ...
    // GIVEROLE COMMAND
        ...
        const targetId = args[0];
        const roleId = args[1];
        if (!targetId || !roleId) return sendError(message, 'Usage: `!giverole <userID> <roleID>`');
    ...
    // CREATEROLE COMMAND
        ...
        await sendLog(message, `Create Role (${role.name})`, { id: 'N/A', tag: role.name });
    }
    ...
    // 8BALL COMMAND
        ...
        const answers = [
            ...
            'Ask again later. ⏰', 'Yes, but be careful ⚠️', 'No, and never ❌'
        ];
    ...
    // HUG COMMAND
        ...
        if (targetId) {
            const found = await getTarget(message, targetId);
